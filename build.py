...
def check_encryptly_runs(timeout: int = 600) -> tuple[bool, str]:
    ...
    try:
        ...
        if not safe_dir.exists():
            return False, "Failed to create safe directory"
...
def _safe_write_diagnostic_report(path, report):
    """Write diagnostic report with error handling."""
    try:
        _safe_write_diagnostic_report(path, report)
        return True
    except (IOError, OSError, ValueError) as e:
        print("  WARNING: Failed to write diagnostic report: " + str(e))
        return False
